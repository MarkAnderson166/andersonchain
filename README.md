...
**Nonce - 'Proof of Work'**  
The mining reward or 'coinbase' is halved every 20 blocks mined and applied to the miner's designated wallet as a transaction in that block. Bitcoin’s halving system is the same, just at much larger chain lengths.
...
* A p2p distributed system such as bitcoin or any other major crypto currency **requires** the 'hashing race'. Incentive is required to maintain the ledger and competition will always spring from that. As these systems are more widely adopted and each block's transaction fees rise it will be increasingly profitable to waste as power at scale. This cannot be sustained.
